...
import {legacySupported} from './legacy.mjs';
...
['YourCharacterRemoved','YourCharacterKOd','YourCharacterOriginalPowerXOrMoreKOd','OpponentCharacterKOd','XMyDonIsReturned','OppActivatesEvent','YouActivateEvent','OppActivatesTrigger','YouActivateTrigger','YouRestedCharacter','YouRemovedCharacter'].forEach(k=>implemented.proc.add(k));
implemented.details.add('CanUseOnPlays');
implemented.effect.add('DeclareCost');
['CharacterPowerXOrMore','CharacterCategoryXPowerOrMore','CharacterCategoryXCostOrMore','ACOCColorReq','HandSentToTrashMyEffect'].forEach(k=>implemented.proc.add(k));
['CostXOrHigherCharacterCategory','OppXMoreOrMoreCharacters','BoardLessThanCostX'].forEach(k=>implemented.details.add(k));
['LifeSentToHand','LifeSentToTrash','LifeSentToDeck','LifeSentToField','YourLifeSentToHand'].forEach(k=>implemented.proc.add(k));
['HideOppHand','RevealOppHand','RevealSelectedHand','ExtraTurn','MatchAttackerBasePowerUntilTurnEnd','OtherCharsImmuneToOpponentNoncombatKO'].forEach(k=>implemented.effect.add(k));
['EffectImmune','FieldGainsTenacity','GainConfusion'].forEach(k=>implemented.effect.add(k));
['CantPlayAnyCardsFromHand','AllDeployRested','NoOnPlays','OppNoOnPlayToTurnEnd','OppNoBlockerThisTurn','FieldRushCharacters','FieldBlocker','FieldUnblockable','FieldCantAttack','FieldCantAttackLeader','MillForXPreviousTargets','BuffSelf1KPerXTargets','BuffXPerGivenDon'].forEach(k=>implemented.effect.add(k));
['LeaderPowerXOrMore','OppLeaderPowerXOrMore','UsoppLeader','StartOfGame','TurnXOrLater','HasBattledCharacter','MyLeaderHit','IHitLeader','OppLosesLife','OppLostLife','DrewDonForTurn','YouDeployed','YouDeployedFromTrash','DeployedAnyFromTrash','MyHandTrashed','OnRestByOpponent'].forEach(k=>implemented.proc.add(k));
['CharacterTypeHas1KCounter','SaveTargetName','FreezeAllCostOrLess','AllyHandPlayCostReduce','MatchLeaderToBasePowerUntilTurnStart','InfiniteReturnToDeck','BuffSelf1KPerXTargets','HealEndOfTurn','KOIfCostXOrLess','VulnerableToNoncombat','PeekOppTopDeckFlip','GainEphemeral','SendPreAllyToTopLife','CantAttackOrigCostOrLess','CantBeDeployedViaEffect','RosinanteLeader','CantDrawFromEffects','ImmuneToRestFromLeaderOrChar','DonDeckOverride','GainStrikeTypes','PassivePowerPerUniqueDeploy','CharactersWithPowerHas2KCounter','CharactersWithNoCounterHaveXCounter','HandCounterChange','ValidTargetsHaveXCounter','ShuffleHandIntoDeck','VulnerableToStrikes','ActivateAllFilmCharsEndOfTurn','ZoroDon','CountsAsEverything','FaceUpLifeGoesToDeck','Passive1CostPerXTrash','DetachDon','CopyTargetsAtEnd','FlipBotLifeDown','FlipBotLifeUp'].forEach(k=>implemented.effect.add(k));
['DifferentColorFromPrevTarget','NameMatchesSaved','TargetCountSavedCount','WillBeRemoved','CostMyLifeOrLess','SecondOriginalCostOrLess','CostOrLessOverride','CostOverrideMyCostXOrMoreExists','SecondOriginalPowerXOrLess','AutoMainTarget'].forEach(k=>implemented.target.add(k));
['DeployedUid','BattleTarget'].forEach(k=>implemented.target.add(k));
['YouBouncedOppCharacter','YourCharacterMatchingTargetKOd','OnOpponentAttackThis','AnyHitLeader','CardTrashedFromMyCategory','OppDeploysOriginalCostXOrMore','OppDeploysThroughCharacter','NoOtherNameWithBaseCostInYourDeploy','AnotherCharacterAllowSameCard','OverrideLeaderStrikeType','OverrideLeaderColorCountName','RogerSilence','YouDeployedTriggerCharacter','OpponentLeaderStrikeTypeRequired','OppPlaysEvent','NamesWithPowerOnYourSide','AddlCharacterCategoryXPowerOrMore','NotCharacterCategoryXCostOrMore','MyLeaderAttacks','MyLeaderAttacked','AllyVanilla','QueuedStartOfMainPhase','PreRestCharacterOnly','AfterFailedKO','NamesWithBasePowerInYourDeploy'].forEach(k=>implemented.proc.add(k));
['OpponentDeployed','AllyCostXOrMore'].forEach(k=>implemented.proc.add(k));
['HasValidTargets','LastTrashedCostXOrMore','AttackerHasStrikeTypes','AttackerHasCardTypes','ActivatedEventXOrMore','DeckOut','AnyOppCharacterKOd','NumTriggersInPlay','NamesWithExactBasePowerInYourDeploy','RequireAll'].forEach(k=>implemented.details.add(k));
implemented.proc.add('DeckOut');
['BuffPreAlly','ImmuneToRemovalFromCharactersXBasePowerOrLess','OpponentFieldImmuneToRemoval','TakeDamage','QueueUpOppMainPhaseAction','BuffXPerTopDeckCost'].forEach(k=>implemented.effect.add(k));
