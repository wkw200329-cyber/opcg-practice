...
export class Engine {
  ...
  start(decks,first=0){
    ...
    this.s={version:2,turn:0,active:first,first,phase:'mulligan',cards:{},players:[],queue:[],prompt:null,battle:null,winner:null,log:[],events:[],eventBatch:0,endings:[],donActivations:[]};this.history=[];this.serial=0;
  ...
  move(c,zone,{bottom=false,rested=false,faceUp=false}={}){
    ...
    if(from==='deck'&&zone!=='deck'&&this.s.phase!=='mulligan'&&!p.deck.length&&this.s.winner===null){
      if(this.hasDeckOutDelay(c.owner)){this.s.deckOutPending??={};this.s.deckOutPending[c.owner]=true;this.log(`${this.label(c.owner)} 的牌库已空：将在本回合结束时败北`);}
      else {this.win(this.flags(this.list(c.owner,'leader')[0]).WinsByDeckout?c.owner:1-c.owner,'牌库已空');throw new GameFinished();}
    }
  }
  draw(owner,n=1,events=true){for(let k=0;k<n;k++){const c=this.list(owner,'deck')[0];if(!c){if(this.hasDeckOutDelay(owner)){this.s.deckOutPending??={};this.s.deckOutPending[owner]=true;this.log(`${this.label(owner)} 的牌库已空：将在本回合结束时败北`);}else this.win(1-owner,'对方牌库已空');break}this.move(c,'hand');if(events)this.emit('CardDrawn',c,{owner});}if(n)this.log(`${this.label(owner)} 抽 ${n} 张牌`);}
  hasDeckOutDelay(owner){return this.board(owner).some(c=>this.actions(c).some(a=>a.proc.DeckOut));}
  ...
  command(cmd){
    ...
    if(cmd.type==='play'){
      ...
      assert(['角色','舞台'].includes(d.type),'不能登场');const cost=this.playCost(c);assert(this.readyDon(owner).length>=cost,'可用 DON!! 不足');
      ...
      this.pay(owner,cost);if(this.discountApplies(c))delete this.s.nextHandDeployDiscount?.[owner];if(d.type==='舞台')for(const old of this.list(owner,'stage'))this.move(old,'trash');this.deploy(c);return;
    }
    if(cmd.type==='attach'){
  ...
  choose(cmd){
    ...
    if(p.type==='triggerOrder'){const i=Number(cmd.value);assert(Number.isInteger(i)&&i>=0&&i<p.entries.length,'请选择一个待结算效果');const entry=p.entries[i],at=this.s.events.indexOf(entry);assert(at>=0,'待结算效果已失效');this.s.events.splice(at,1);this.s.queue.unshift(entry);return}
  ...
  deploy(c,rested=false){if(!this.canDeploy(c)){this.log(`${this.name(c)}：受本回合效果限制，无法登场`);return}const z=this.def(c).type==='舞台'?'stage':'field';rested||=!!this.s.allDeployRested?.[c.owner];if(z==='field'&&this.list(c.owner,z).length>=5&&c.zone!=='field'){this.s.interruptions??=[];this.s.interruptions.push({kind:'effectDeployment',uid:c.uid,rested});return}if(z==='stage')for(const x of this.list(c.owner,z))this.move(x,'trash');const from=c.zone;this.move(c,z,{rested});this.log(`${this.label(c.owner)} 登场 ${this.name(c)}`);const deployed={owner:c.owner,from,uid:c.uid};this.emit('Deployed',c,{deployed});this.emit('OpponentDeployed',c,{owner:c.owner,deployed});if(!this.s.noOnPlay?.[c.owner])this.emit('OnPlay',c,{owner:c.owner});}
  attackReason(c,t){
  ...
  emit(event,subject,context={}){
    ...
    const conditionEvents={CharacterRemoved:['YourCharacterRemoved','YourCharacterKOd','YourCharacterOriginalPowerXOrMoreKOd','OpponentCharacterKOd','YouRemovedCharacter'],MyDonIsReturned:['XMyDonIsReturned'],ActivatesEvent:['OppActivatesEvent','YouActivateEvent'],ActivatesTrigger:['OppActivatesTrigger','YouActivateTrigger'],YouRestedCharacter:['YouRestedCharacter','OnRestByOpponent'],HandSentToTrash:['HandSentToTrashMyEffect','MyHandTrashed'],LifeSent:['LifeSentToHand','LifeSentToTrash','LifeSentToDeck','LifeSentToField','YourLifeSentToHand'],Deployed:['YouDeployed','YouDeployedFromTrash','DeployedAnyFromTrash'],OpponentDeployed:['OpponentDeployed']}[event]||[];
    const sources=direct.includes(event)?(subject?[subject]:[]):[...this.board(owner??0),...this.board(1-(owner??0))];
    const batch=++this.s.eventBatch;
    for(const c of sources)this.actions(c).forEach((a,index)=>{if((a.proc[event]||conditionEvents.some(k=>a.proc[k]))&&(direct.includes(event)||event==='AnyCharacterKOd'||conditionEvents.length||c.owner===owner)&&this.conditions(a.proc,c,context)&&(!a.proc.OncePerTurn||c.used[index]!==this.s.turn))this.s.events.push({kind:'offer',uid:c.uid,index,context,batch,event});});
  }
  pump(){let n=0;while(!this.s.prompt&&(this.s.queue.length||this.s.events.length||this.s.interruptions?.length)&&this.s.winner===null){assert(++n<1000,'效果循环超过安全上限');if(this.s.interruptions?.length){this.s.queue.unshift(...this.s.interruptions);this.s.interruptions=[];}else if(this.s.events.length&&!['effect','effectDeployment','offer'].includes(this.s.queue[0]?.kind)){this.offerNextTrigger();if(this.s.prompt)break;}const t=this.s.queue.shift();if(t)this.task(t)}}
  offerNextTrigger(){const batch=this.s.events[0]?.batch,entries=this.s.events.filter(x=>x.batch===batch),owners=[this.s.active,1-this.s.active];const owner=owners.find(o=>entries.some(x=>this.s.cards[x.uid]?.owner===o));const mine=entries.filter(x=>this.s.cards[x.uid]?.owner===owner);if(mine.length>1){this.ask({type:'triggerOrder',owner,entries:mine,title:'多个效果同时触发：选择下一个结算的效果'});return}const entry=mine[0];if(entry){this.s.events.splice(this.s.events.indexOf(entry),1);this.s.queue.unshift(entry);}}
  ...
  task(t){
    ...
    if(t.kind==='finishTurn'){const o=this.s.active;if(this.s.deckOutPending?.[o]&&!this.list(o,'deck').length){this.win(1-o,'对方牌库为零，回合结束时败北');return}for(const c of Object.values(this.s.cards))c.mods=c.mods.filter(m=>m.until==='ownerStart'||m.until==='ownerEnd'&&m.owner!==o||m.until==='oppEnd'&&m.owner===o);if(this.s.noActivateDon)delete this.s.noActivateDon[o];delete this.s.nextHandDeployDiscount?.[o];this.s.noPlayCharacters={};this.s.noPlayOriginalCost={};const extra=this.s.extraTurnOwner===o;delete this.s.extraTurnOwner;this.s.active=extra?o:1-o;this.s.queue.push({kind:'beginTurn'});return}
    ...
    if(t.kind==='battleEnd'){const b=this.s.battle;if(b&&this.def(this.s.cards[b.target]).type==='角色')this.emit('AfterBattleCharacter',this.s.cards[b.attacker],{owner:this.s.cards[b.attacker].owner,battleTarget:b.target});for(const c of Object.values(this.s.cards))c.mods=c.mods.filter(m=>m.until!=='battle');this.s.battle=null;return}
  ...
  matches(c,t,source,ctx={}){
    ...
    if(t.DeployedUid&&c.uid!==ctx.context?.deployed?.uid)return false;
    if(t.BattleTarget&&c.uid!==ctx.context?.battleTarget)return false;
    if(t.DifferentColorFromPrevTarget){const prev=this.s.cards[ctx.previous?.[0]];if(!prev||overlaps(r.cardColors,this.rule(prev).cardColors))return false;}
    ...
    if(t.CostOrLess&&!(t.CostOrLessIfTrashXOrMore&&this.list(o,'trash').length>=t.CostOrLessIfTrashXOrMore.trash)&&this.cost(c)>t.CostOrLess||t.CostOrLessIfTrashXOrMore&&this.list(o,'trash').length>=t.CostOrLessIfTrashXOrMore.trash&&this.cost(c)>t.CostOrLessIfTrashXOrMore.cost||t.CostOppLifeOrLess&&this.cost(c)>this.list(1-o,'life').length||t.CostCombinedLifeOrLess&&this.cost(c)>this.list(o,'life').length+this.list(1-o,'life').length||t.CostDonOrLess&&this.cost(c)>this.don(o).length||t.CostOppDonOrLess&&this.cost(c)>this.don(1-o).length||t.GivenDon&&this.attached(c).length<t.GivenDon||t.NotNames?.includes(r.characterName))return false;
    ...
    if(t.CostOrLess&&!(t.CostOrLessIfTrashXOrMore&&this.list(o,'trash').length>=t.CostOrLessIfTrashXOrMore.trash)&&this.cost(c)>t.CostOrLess||t.CostOrMore&&this.cost(c)<t.CostOrMore||t.OriginalCostOrLess&&(d.cost||0)>t.OriginalCostOrLess||t.OriginalCostOrMore&&(d.cost||0)<t.OriginalCostOrMore||t.CostZero&&this.cost(c)!==0)return false;
  ...
  passive(c,wanted=null){const out=[];for(const source of [...this.board(0),...this.board(1)])for(const a of this.actions(source))if(a.proc.Passive&&(!wanted||a.steps?.some(s=>wanted.some(k=>s.effect?.[k])))&&this.conditions(a.proc,source))for(const step of a.steps||[]){const ef=step.effect||{};if(wanted&&!wanted.some(k=>ef[k]))continue;if(!this.conditions(step.details||{},source))continue;const field=Object.keys(ef).some(k=>k.startsWith('Field')||k.startsWith('Ally')||k.startsWith('Opponent')||k.startsWith('OtherChars'));if(field){if(ef.AllyFieldCostChange&&(c.zone!=='field'||c.owner!==source.owner)||ef.OpponentFieldCostChange&&(c.zone!=='field'||c.owner===source.owner)||ef.AllyHandCostChange&&(c.zone!=='hand'||c.owner!==source.owner))continue;if(step.target?.length?step.target.some(t=>this.matchesPassive(c,t,source)):(ef.OpponentFieldCostChange?c.owner!==source.owner:c.owner===source.owner))out.push({source,effect:ef})}else if(source.uid===c.uid)out.push({source,effect:ef})}return out;}
  matchesPassive(c,t,source){const r=this.rule(c),d=this.def(c);return (!t.FriendlyOnly||c.owner===source.owner)&&(!t.EnemyOnly||c.owner!==source.owner)&&(!t.NotSelf||c.uid!==source.uid)&&(!t.ActiveOnly||!c.rested)&&(!t.RestedOnly||c.rested)&&(!t.OnlyTypes||t.OnlyTypes.includes(r.cardType))&&(!t.OnlyColors||overlaps(r.cardColors,t.OnlyColors))&&(!t.OnlyStrikeTypes||t.OnlyStrikeTypes.includes(r.strikeType))&&(!t.OnlyCategories||overlaps(r.cardCategories,t.OnlyCategories))&&(!t.OnlyNames||t.OnlyNames.includes(r.characterName))&&(!t.OriginalCostOrLess||(d.cost||0)<=t.OriginalCostOrLess)&&(!t.OriginalCostOrMore||(d.cost||0)>=t.OriginalCostOrMore)&&(!t.DeployedCharacter&&!t.Leader||t.DeployedCharacter&&c.zone==='field'||t.Leader&&c.zone==='leader');}
  basePower(c){let n=this.def(c).power||0;for(const m of c.mods||[])if(m.key==='basePower')n=m.value;return n;}
  power(c){let n=this.basePower(c);for(const m of c.mods||[])if(m.key==='power')n+=m.value;if(this.s.active===c.owner)n+=1000*this.attached(c).length;for(const {effect:e}of this.passive(c)){if(e.FieldBasePowerChange!==undefined)n=e.FieldBasePowerChange;if(e.PassiveBasePowerMatchLeader)n=this.basePower(this.list(c.owner,'leader')[0]);n+=(e.PassivePowerChange||0)+(e.FieldPowerBuff||0)-(e.FieldPowerDebuff||0);if(e.Passive1KPerXTrash)n+=Math.floor(this.list(c.owner,'trash').length/e.Passive1KPerXTrash)*1000;if(e.Passive1KPerXEventTrash)n+=Math.floor(this.list(c.owner,'trash').filter(x=>this.def(x).type==='事件').length/e.Passive1KPerXEventTrash)*1000;if(e.Passive1KPerXRestedDon)n+=Math.floor(this.restDon(c.owner).length/e.Passive1KPerXRestedDon)*1000;if(e.Passive1KPerXHand)n+=Math.floor(this.list(c.owner,'hand').length/e.Passive1KPerXHand)*1000;}for(const m of c.mods||[])if(m.key==='powerSet')n=m.value;return Math.max(0,n);}
  cost(c){let n=this.def(c).cost||0;for(const m of c.mods||[])if(m.key==='cost')n+=m.value;for(const {effect:e}of this.passive(c,['PassiveCostChange','AllyFieldCostChange','OpponentFieldCostChange','AllyHandCostChange','Passive2CostPerXTrash'])){n+=(e.PassiveCostChange||0)+(e.AllyFieldCostChange||0)+(e.OpponentFieldCostChange||0)+(e.AllyHandCostChange||0);if(e.Passive2CostPerXTrash)n-=Math.floor(this.list(c.owner,'trash').length/e.Passive2CostPerXTrash)*2;}if(c.zone==='hand')for(const a of this.actions(c)){const e=a.steps?.[0]?.effect||{};if(a.proc.Passive&&this.conditions(a.proc,c)&&this.conditions(a.steps[0].details||{},c)){n+=e.HandCostChange||0;if(e.Passive2CostPerXTrash)n-=Math.floor(this.list(c.owner,'trash').length/e.Passive2CostPerXTrash)*2;}}return Math.max(0,n);}
  discountApplies(c){const d=this.s.nextHandDeployDiscount?.[c.owner],r=this.rule(c);return !!d&&c.zone==='hand'&&this.def(c).type==='角色'&&(this.def(c).cost||0)>=d.minCost&&overlaps(r.cardCategories,d.categories);}
  playCost(c){return Math.max(0,this.cost(c)-(this.discountApplies(c)?this.s.nextHandDeployDiscount[c.owner].amount:0));}
  counter(c){return this.def(c).counter||0;}
  flags(c){let f={};for(const {effect}of this.passive(c))Object.assign(f,effect);for(const m of c.mods||[])if(m.key==='flag')f[m.value]=true;if(f.FieldDoubleAttack)f.DoubleAttack=true;if(f.FieldBlocker)f.Blocker=true;if(f.FieldRushCharacters)f.Rush=true;if(f.OtherCharsImmuneToNoncombatKO)f.ImmuneToNoncombat=true;if(f.OtherCharsImmuneToOpponentNoncombatKO)f.ImmuneToOpponentNoncombat=true;if(f.OtherCharsImmuneToBattle)f.ImmuneToBattle=true;if(f.LoseBlocker)f.Blocker=false;return f;}
  ...
  effectStep(t){
    ...
    if(e.Choices?.length&&!t.choiceDone){const choices=e.Choices.filter(choice=>!choice.When||this.conditions(choice.When,c,{...t.context,previous:t.previous}));if(choices.length){this.ask({type:'choice',owner:e.ForceOpponent?1-c.owner:c.owner,choices,task:t,title:'选择一种效果'});return}}
  ...
  applyEffects(e,c,targets,t){
    ...
    if(e.DrawCards)this.draw(o,e.DrawCards);if(e.DrawIfAllyCategoryInPlay&&this.list(o,'field').some(x=>overlaps(this.rule(x).cardCategories,e.DrawIfAllyCategoryInPlay)))this.draw(o,1);if(e.ArmNextHandDeployDiscount)this.s.nextHandDeployDiscount??={},this.s.nextHandDeployDiscount[o]=e.ArmNextHandDeployDiscount;if(e.DrawTo)this.draw(o,Math.max(0,e.DrawTo-this.list(o,'hand').length));
    ...
    if(e.CantPlayAnyCardsFromHand){this.s.noPlayFromHand??={};this.s.noPlayFromHand[o]=true;}if(e.AllDeployRested){this.s.allDeployRested??={};this.s.allDeployRested[o]=true;}if(e.NoOnPlays){this.s.noOnPlay??={};this.s.noOnPlay[o]=true;}if(e.OppNoOnPlayToTurnEnd){this.s.noOnPlay??={};this.s.noOnPlay[1-o]=true;}if(e.OppNoBlockerThisTurn){this.s.noBlocker??={};this.s.noBlocker[1-o]=true;}
    if(e.FieldRushCharacters)for(const x of this.list(o,'field'))this.mod(x,'flag','Rush','ownerEnd');if(e.FieldBlocker)for(const x of this.list(o,'field'))this.mod(x,'flag','Blocker','ownerEnd');if(e.FieldUnblockable)for(const x of this.list(o,'field'))this.mod(x,'flag','Unblockable','ownerEnd');if(e.FieldCantAttack)for(const x of this.list(o,'field'))this.mod(x,'flag','CantAttack','ownerEnd');if(e.FieldCantAttackLeader)for(const x of this.list(o,'field'))this.mod(x,'flag','CantAttackLeader','ownerEnd');
    if(e.MillForXPreviousTargets)for(const x of this.list(o,'deck').slice(0,(t.previous||[]).length))this.move(x,'trash');if(e.BuffSelf1KPerXTargets)this.mod(c,'power',1000*(t.previous||[]).length,'ownerEnd');if(e.BuffXPerGivenDon)this.mod(c,'power',e.BuffXPerGivenDon*this.attached(c).length,'ownerEnd');
    if(e.MatchLeaderToBasePowerUntilTurnStart)this.mod(this.list(o,'leader')[0],'basePower',e.MatchLeaderToBasePowerUntilTurnStart,'ownerStart');if(e.FreezeAllCostOrLess)for(const x of this.list(1-o,'field').filter(x=>this.cost(x)<=e.FreezeAllCostOrLess))x.freeze=true;if(e.InfiniteReturnToDeck)for(const x of targets)this.move(x,'deck');if(e.KOIfCostXOrLess)for(const x of targets.filter(x=>this.cost(x)<=e.KOIfCostXOrLess))this.remove(x,'trash',false,c);if(e.HealEndOfTurn){this.s.endHeals??=[];this.s.endHeals.push({owner:o,count:e.HealEndOfTurn,turn:this.s.turn});}if(e.GainEphemeral)this.mod(c,'flag','Ephemeral','ownerEnd');
    if(e.SaveTargetName&&targets[0])t.savedTargetName=this.rule(targets[0]).characterName;if(e.SendPreAllyToTopLife)for(const u of t.previous||[]){const x=this.s.cards[u];if(x?.owner===o&&x.zone==='field')this.move(x,'life');}if(e.CantAttackOrigCostOrLess)for(const x of targets)this.mod(x,'flag','CantAttack','ownerEnd');
    if(e.RevealOppHand){this.s.revealedHands??={};this.s.revealedHands[1-o]=true;this.log(`${this.label(1-o)} 的手牌已公开`);}if(e.RevealSelectedHand){this.s.revealedCards??={};for(const x of targets)this.s.revealedCards[x.uid]=true;this.log(`${this.label(1-o)} 的 ${targets.length} 张手牌已公开`);}if(e.HideOppHand&&this.s.revealedHands){delete this.s.revealedHands[1-o];this.log(`${this.label(1-o)} 的手牌不再公开`);}
    if(e.ExtraTurn)this.s.extraTurnOwner=o;
    if(e.MatchAttackerBasePowerUntilTurnEnd){const attacker=this.s.cards[t.context?.attacker];if(attacker)this.mod(c,'basePower',this.def(attacker).power||0,'ownerEnd');}
    ...
    for(const x of targets){
      ...
      if(e.AttachRestedDon||e.AttachActiveDon){const ds=e.AttachRestedDon?this.restDon(o):this.readyDon(o),count=Number(e.AttachRestedDon||e.AttachActiveDon)||1;for(const d of ds.slice(0,count))d.attached=x.uid;}
