...
export class Engine {
  ...
  deploy(c,rested=false){if(!this.canDeploy(c)){this.log(`${this.name(c)}：受本回合效果限制，无法登场`);return}const z=this.def(c).type==='舞台'?'stage':'field';rested||=!!this.s.allDeployRested?.[c.owner];if(z==='field'&&this.list(c.owner,z).length>=5&&c.zone!=='field'){this.s.interruptions??=[];this.s.interruptions.push({kind:'effectDeployment',uid:c.uid,rested});return}if(z==='stage')for(const x of this.list(c.owner,z))this.move(x,'trash');const from=c.zone;this.move(c,z,{rested});this.log(`${this.label(c.owner)} 登场 ${this.name(c)}`);this.emit('Deployed',c,{deployed:{owner:c.owner,from,uid:c.uid}});if(!this.s.noOnPlay?.[c.owner])this.emit('OnPlay',c,{owner:c.owner});}
  attackReason(c,t){
  ...
  task(t){
    ...
    if(t.kind==='finishTurn'){const o=this.s.active;for(const c of Object.values(this.s.cards))c.mods=c.mods.filter(m=>m.until==='ownerStart'||m.until==='ownerEnd'&&m.owner!==o||m.until==='oppEnd'&&m.owner===o);if(this.s.noActivateDon)delete this.s.noActivateDon[o];delete this.s.nextHandDeployDiscount?.[o];this.s.noPlayCharacters={};this.s.noPlayOriginalCost={};this.s.active=1-o;this.s.queue.push({kind:'beginTurn'});return}
  ...
  flags(c){let f={};for(const {effect}of this.passive(c))Object.assign(f,effect);for(const m of c.mods||[])if(m.key==='flag')f[m.value]=true;if(f.FieldDoubleAttack)f.DoubleAttack=true;if(f.FieldBlocker)f.Blocker=true;if(f.FieldRushCharacters)f.Rush=true;if(f.OtherCharsImmuneToNoncombatKO)f.ImmuneToNoncombat=true;if(f.OtherCharsImmuneToBattle)f.ImmuneToBattle=true;if(f.LoseBlocker)f.Blocker=false;return f;}
