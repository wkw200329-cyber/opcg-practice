...
export class Engine {
  ...
  flags(c){let f={};for(const {effect}of this.passive(c))Object.assign(f,effect);for(const m of c.mods||[])if(m.key==='flag')f[m.value]=true;if(f.FieldDoubleAttack)f.DoubleAttack=true;if(f.FieldBlocker)f.Blocker=true;if(f.FieldRushCharacters)f.Rush=true;if(f.OtherCharsImmuneToNoncombatKO)f.ImmuneToNoncombat=true;if(f.LoseBlocker)f.Blocker=false;return f;}
