...
export class Engine {
  ...
  matches(c,t,source,ctx={}){
    ...
    if(t.CostOrLess&&this.cost(c)>t.CostOrLess||t.CostOppLifeOrLess&&this.cost(c)>this.list(1-o,'life').length||t.CostDonOrLess&&this.cost(c)>this.don(o).length||t.GivenDon&&this.attached(c).length<t.GivenDon||t.NotNames?.includes(r.characterName))return false;
