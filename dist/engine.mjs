...
export class Engine {
  ...
  command(cmd){
    ...
    if(cmd.type==='play'){
      ...
      if(d.type==='角色'&&this.list(owner,'field').length===5){this.ask({type:'replace',owner,candidates:this.s.players[owner].field.slice(),min:1,max:1,card:c.uid,title:'角色区已满，选择一张角色丢弃'});return}
  ...
  choose(cmd){
    ...
    if(p.type==='block'){if(picks.length){const c=this.s.cards[picks[0]];assert(!c.rested&&this.flags(c).Blocker,'角色不能阻挡');c.rested=true;this.s.battle.target=c.uid;this.s.battle.blocked=true;this.log(`${this.name(c)} 发动阻挡`);this.emit('OnBlock',c,{owner:c.owner});this.emit('OpponentActivatesBlocker',c,{owner:1-c.owner,blocker:c.uid})}this.s.queue.push({kind:'counter'});return}
    if(p.type==='counter'){
  ...
  emit(event,subject,context={}){
    ...
    const direct=['OnPlay','OnAttack','OnAttackLeader','OnBlock','OnKO','OnKOEffectOnly','AfterBattleCharacter'];
  ...
  task(t){
    ...
    if(t.kind==='beginTurn'){
      ...
      for(const c of this.board(o)){if(!c.freeze)c.rested=false;delete c.freeze;for(const d of this.attached(c))delete d.attached;c.mods=c.mods.filter(m=>!(m.until==='ownerStart'&&m.owner===o));}
  ...
  conditions(p,c,context={}){
    ...
    for(const[k,v]of Object.entries(p)){
      ...
      if(v&&typeof v==='object'&&!Array.isArray(v)&&v.iCount){const list=this.list(o,'field');if(k.includes('Category')&&!list.some(x=>overlaps(this.rule(x).cardCategories,[v.eCategory])&&(k.includes('Rested')?x.rested:true)))return false;}
    }return true;
  }
  ...
  matches(c,t,source,ctx={}){
    ...
    if(t.CostOrLess&&this.cost(c)>t.CostOrLess||t.NotNames?.includes(r.characterName))return false;
  ...
  passive(c,wanted=null){const out=[];for(const source of [...this.board(0),...this.board(1)])for(const a of this.actions(source))if(a.proc.Passive&&(!wanted||a.steps?.some(s=>wanted.some(k=>s.effect?.[k])))&&this.conditions(a.proc,source))for(const step of a.steps||[]){const ef=step.effect||{};if(wanted&&!wanted.some(k=>ef[k]))continue;if(!this.conditions(step.details||{},source))continue;const field=Object.keys(ef).some(k=>k.startsWith('Field')||k.startsWith('Ally')||k.startsWith('Opponent'));if(field){if(ef.AllyFieldCostChange&&(c.zone!=='field'||c.owner!==source.owner)||ef.OpponentFieldCostChange&&(c.zone!=='field'||c.owner===source.owner))continue;if(step.target?.length?step.target.some(t=>this.matchesPassive(c,t,source)):(ef.OpponentFieldCostChange?c.owner!==source.owner:c.owner===source.owner))out.push({source,effect:ef})}else if(source.uid===c.uid)out.push({source,effect:ef})}return out;}
  ...
  power(c){let n=this.def(c).power||0;for(const m of c.mods||[])if(m.key==='basePower')n=m.value;for(const m of c.mods||[])if(m.key==='power')n+=m.value;if(this.s.active===c.owner)n+=1000*this.attached(c).length;for(const {effect:e}of this.passive(c))n+=(e.PassivePowerChange||0)+(e.FieldPowerBuff||0)-(e.FieldPowerDebuff||0);return Math.max(0,n);}
  cost(c){let n=this.def(c).cost||0;for(const m of c.mods||[])if(m.key==='cost')n+=m.value;for(const {effect:e}of this.passive(c,['PassiveCostChange','AllyFieldCostChange','OpponentFieldCostChange']))n+=(e.PassiveCostChange||0)+(e.AllyFieldCostChange||0)+(e.OpponentFieldCostChange||0);if(c.zone==='hand')for(const a of this.actions(c))if(a.proc.Passive&&a.steps?.[0]?.effect?.HandCostChange&&this.conditions(a.proc,c)&&this.conditions(a.steps[0].details||{},c))n+=a.steps[0].effect.HandCostChange;return Math.max(0,n);}
  counter(c){return this.def(c).counter||0;}
  flags(c){let f={};for(const {effect}of this.passive(c))Object.assign(f,effect);for(const m of c.mods||[])if(m.key==='flag')f[m.value]=true;if(f.LoseBlocker)f.Blocker=false;return f;}
  ...
  effectStep(t){
    ...
    while(t.group<ts.length){const target=ts[t.group];let candidates=Object.values(this.s.cards).filter(x=>this.matchesGroup(x,step,t.group,c,t));if(target.AutoSelf)candidates=candidates.filter(x=>x.uid===c.uid);if(target.AutoCopyPreviousTargets)candidates=t.previous.map(u=>this.s.cards[u]);
      ...
      const requiredCount=e.DonMinusToOppCount?e.DonMinus:e.DonMinus||target.TargetCount||1;if(requiredCount===0){t.targets[t.group]=[];t.group++;continue}
  ...
  applyEffects(e,c,targets,t){
    ...
    if(e.RestSelf){assert(!c.rested,'该卡已横置');c.rested=true}
    ...
    for(const x of targets){
      ...
      if(e.Activate)x.rested=false;if(e.Rest)x.rested=true;if(e.Freeze)x.freeze=true;if(e.FlipLifeDown)x.faceUp=false;
