...
export class Engine {
  ...
  power(c){let n=this.def(c).power||0;for(const m of c.mods||[])if(m.key==='basePower')n=m.value;for(const m of c.mods||[])if(m.key==='power')n+=m.value;if(this.s.active===c.owner)n+=1000*this.attached(c).length;for(const {effect:e}of this.passive(c)){if(e.FieldBasePowerChange!==undefined)n=e.FieldBasePowerChange;n+=(e.PassivePowerChange||0)+(e.FieldPowerBuff||0)-(e.FieldPowerDebuff||0);}return Math.max(0,n);}
